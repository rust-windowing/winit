            ...
            WindowEvent::KeyboardInput { event, is_synthetic: false, .. } => {
                ...
                if event.state.is_pressed() {
                    if mods == ModifiersState::CONTROL {
                        ...
                        // selection.
                        info!("{}", pretty_with_cursor(text, *cursor, *cursor));
                    }
                }
            },
            _ => (),
...
fn pretty_with_cursor(text: &str, cursor: usize, anchor: usize) -> String {
    pretty_checked(text, cursor, anchor).unwrap_or_else(|e| format!("INVALID: {e}"))
}

fn pretty_checked(text: &str, cursor: usize, anchor: usize) -> Result<String, &str> {
