...
impl EventLoopRunner {
    ...
    pub(super) fn clipboard_window(&self) -> Option<WindowId> {
        let windows = self.windows.borrow();
        let foreground = WindowId::from_raw(unsafe { GetForegroundWindow() } as usize);

        windows.contains(&foreground).then_some(foreground).or_else(|| windows.first().copied())
    }
