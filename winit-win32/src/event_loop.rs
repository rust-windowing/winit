...
impl RootActiveEventLoop for ActiveEventLoop {
    ...
    fn fetch_data_transfer(
        ...
        let (data, window_id) = match self.0.data_transfer().drag_state(id) {
            Some(state) => (state.data.clone(), state.window_id),
            None => {
                let data = self.clipboard_by_id(id).ok_or(RequestError::Ignored)?;
                let window_id = self.0.clipboard_window().ok_or(RequestError::Ignored)?;
                (data, window_id)
            },
        };
