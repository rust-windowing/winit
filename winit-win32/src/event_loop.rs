...
use winit_core::event::{
    DeviceButtonSource, DeviceEvent, DeviceId, FingerId, Force, Ime, RawKeyEvent,
    SurfaceSizeWriter, TabletToolButton, TabletToolData, TabletToolKind, TabletToolTilt,
    ...
    if data.header.dwType == RIM_TYPEMOUSE {
        ...
        for (mouse_button, state) in button_state.into_iter().flatten() {
            userdata.send_device_event(device_id, PointerButton {
                button: DeviceButtonSource::Mouse(mouse_button),
                button_id: 0,
                state,
            });
        }
