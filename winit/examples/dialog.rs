//! This example shows the capabilities of dialogs
//! press 'N' to create a non modal dialog and 'M' to create a modal dialog
        ...
        fn window_event(
            ...
            match event {
                WindowEvent::CloseRequested => {
                    ...
                    self.dialogs.retain_mut(|id| id != &window_id);
                    if self.main_window.unwrap() == window_id || self.windows.is_empty() {
                        event_loop.exit();
                    }
                },
                ...
                WindowEvent::KeyboardInput {
                    ...
                    match code {
                        KeyCode::KeyN | KeyCode::KeyM => {
                            let window_id = if let Some(dialog_id) = self.dialogs.last() {
                                dialog_id
                            } else {
                            ...
                            let child_window = spawn_dialog(
                                parent_window.surface.window().as_ref(),
                                event_loop,
                                child_index,
                                matches!(code, KeyCode::KeyM),
                            );
                            let child_id = child_window.id();
                            self.dialogs.push(child_id);
                            self.windows.insert(
                                child_id,
