...
pub trait PopupExtWayland {
    ...
    fn set_anchor(&mut self, anchor: PopupAnchor);
    ...
    fn set_constraint_adjustment(&mut self, constraint_adjustment: PopupConstraintAdjustment);

    /// Sets the direction the popup surface extends from the anchor point.
    ///
    /// See [`PopupGravity`] for the available directions.
    fn set_gravity(&mut self, gravity: PopupGravity);
}
