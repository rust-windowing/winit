...
use crate::{WindowAttributesWayland, layer_from_window_level, output};
...
impl Window {
    pub(crate) fn new(
        ...
        let wl_attrs =
            attributes.platform.take().and_then(|p| p.cast::<WindowAttributesWayland>().ok());
        let (app_name, activation_token, prefer_csd) = match wl_attrs.clone() {
            Some(attrs) => (attrs.name, attrs.activation_token, attrs.prefer_csd),
            ...
            || attributes.window_level != WindowLevel::Normal
        {
            ...
            if let Some(anchor) = wl_attrs.anchor {
                layer_surface.set_anchor(anchor);
            }
            if let Some(exclusive_zone) = wl_attrs.exclusive_zone {
                layer_surface.set_exclusive_zone(exclusive_zone)
            }
            if let Some((top, right, bottom, left)) = wl_attrs.margin {
                layer_surface.set_margin(top, right, bottom, left);
            }
            if let Some(keyboard_interactivity) = wl_attrs.keyboard_interactivity {
                layer_surface.set_keyboard_interactivity(keyboard_interactivity);
            }
            if let Some((pos, size)) = wl_attrs.region {
                let region = Region::new(compositor.as_ref()).map_err(|err| os_error!(err))?;
            ...
            if let Some(name) = app_name.map(|n| n.general) {
...
impl CoreWindow for Window {
    ...
    fn set_window_level(&self, level: WindowLevel) {
        self.window.set_layer(layer_from_window_level(level));
    }
...
impl WindowShell {
    ...
    pub fn set_maximized(&self) {
        match self {
            WindowShell::Xdg { window } => window.set_maximized(),
    ...
    pub fn unset_maximized(&self) {
        match self {
            WindowShell::Xdg { window } => window.unset_maximized(),
            WindowShell::WlrLayer { .. } => {
                warn!("Unset Maximize is ignored for layer_shell windows")
            },
        }
    }
