...
impl AppState {
    ...
    pub fn clipboard_window(&self, mtm: MainThreadMarker) -> Option<WindowId> {
        let windows = self.windows.borrow();

        windows
            .iter()
            .find_map(|(id, window)| window.get(mtm).load()?.has_focus().then_some(*id))
            .or_else(|| windows.iter().find_map(|(id, window)| window.get(mtm).load().map(|_| *id)))
    }
}
