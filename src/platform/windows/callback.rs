...
/// There's no parameters passed to the callback function, so it needs to get its context (the
/// HWND, the Sender for events, etc.) stashed in a thread-local variable.
...
pub unsafe extern "system" fn callback(window: winapi::HWND,
    ...
    match msg {
        ...
        // Prevents default windows menu hotkeys playing unwanted "ding" sounds. Alternatively
        // could check for WM_SYSCOMMAND with wparam being SC_KEYMENU, but this may prevent some
        ...
        winapi::WM_XBUTTONDOWN => {
            ...
            use events::ElementState::Pressed;
            // waiting on PR for winapi to add GET_XBUTTON_WPARAM
        ...
        winapi::WM_INPUT => {
            ...
            user32::GetRawInputData(mem::transmute(lparam),
                                    winapi::RID_INPUT,
        ...
        winapi::WM_DROPFILES => {
            ...
            for i in 0..num_drops {
                let nch = shell32::DragQueryFileW(hdrop,
                                                  i,
                                                  pathbuf.as_mut_ptr(),
                                                  winapi::MAX_PATH as u32) as
                          usize;
                if nch > 0 {
                    send_event(window,
                               DroppedFile(OsString::from_wide(&pathbuf[0..nch]).into()));
                }
            }
        ...
        winapi::WM_GETMINMAXINFO => {
            ...
            //(*mmi).max_position = winapi::POINT { x: -8, y: -8 };
            // The dimensions of the primary monitor.
            //(*mmi).max_size = winapi::POINT { x: .., y: .. };

            CONTEXT_STASH.with(|context_stash| match context_stash.borrow().as_ref() {
                Some(cstash) => {
