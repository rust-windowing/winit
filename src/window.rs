...
pub enum CursorIcon {
    ...
    /// Represents a custom cursor which uses an image. At the current time, it's only implemented for Windows. The argument has to be a path pointing to a .cur or .ani file.
    Custom(&'static str),
}
