...
impl RunLoop {
    ...
    unsafe fn add_observer(
        ...
        let observer =
            unsafe { CFRunLoopObserver::new(None, flags.0, true, priority, handler, context) }
                .unwrap();
        unsafe { CFRunLoop::add_observer(&self.0, Some(&observer), kCFRunLoopCommonModes) };
    }
    ...
    ///
    /// 1. Using `CFRunLoop::perform_block` or `-[NSRunLoop performBlock:]`.
    ///
    ...
    pub fn queue_closure(&self, closure: impl FnOnce() + 'static) {
        ...
        unsafe { CFRunLoop::perform_block(&self.0, Some(mode), Some(&block)) }
    }
}
...
impl EventLoopWaker {
    pub(crate) fn new() -> Self {
        ...
        unsafe {
            ...
            .unwrap();
            CFRunLoop::add_timer(&CFRunLoop::main().unwrap(), Some(&timer), kCFRunLoopCommonModes);
