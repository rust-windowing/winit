...
use objc2_core_foundation::{
    CFArrayGetCount, CFArrayGetValueAtIndex, CFRetained, CFUUIDGetUUIDBytes, CFUUID,
};
#[allow(deprecated)]
...
impl MonitorHandle {
    ...
    fn uuid(&self) -> u128 {
        u128::from_ne_bytes(unsafe { CFUUIDGetUUIDBytes(&self.0) }.into())
    }
    ...
    pub fn video_mode_handles(&self) -> impl Iterator<Item = VideoModeHandle> {
        ...
        unsafe {
            let modes = {
                let array = CGDisplayCopyAllDisplayModes(self.display_id(), None)
...
#[cfg(test)]
mod tests {
    use objc2_core_foundation::CFUUIDCreate;
    ...
    fn invalid_monitor_handle() {
        ...
        let handle = MonitorHandle(unsafe { CFUUIDCreate(None) }.unwrap());
