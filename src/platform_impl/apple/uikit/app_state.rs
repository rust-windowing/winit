...
impl AppState {
    pub(crate) fn setup_global(mtm: MainThreadMarker) {
        ...
        GLOBAL
            ...
            .expect(
                "application state can only be set once\nNOTE: only one `EventLoop` is supported \
                 on iOS. `EventLoopProxy` might be helpful",
            );
    }
