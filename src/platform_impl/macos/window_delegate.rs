...
declare_class!(
    ...
    unsafe impl WinitWindowDelegate {
        ...
        fn effective_appearance_did_change(&self, _: id) {
            trace_scope!("Triggered `effectiveAppearDidChange:`");
            unsafe {
                let _: () = msg_send![self, performSelectorOnMainThread: sel!(effectiveAppearanceDidChangedOnMainThread:) withObject:nil waitUntilDone:false];
            }
        }
        #[sel(effectiveAppearanceDidChangedOnMainThread:)]
        fn effective_appearance_did_changed_on_main_thread(&self, _: id) {
            self.with_state(|state| {
            ...
            trace_scope!("Completed `effectiveAppearDidChange:`");
        }
    }
);
