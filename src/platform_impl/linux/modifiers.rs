...
use super::x11::{ffi as x11_ffi, xdisplay::XConnection};
...
impl ModifierKeymap {
    ...
    pub fn get_modifier(&self, keycode: u32) -> Option<Modifier> {
    ...
    pub fn reset_from_x_keymap(&mut self, keymap: &x11_ffi::XModifierKeymap) {
    ...
    fn read_x_keys(
        ...
        for &keycode in &keys[start..end] {
            if keycode != 0 {
                self.keys.insert(keycode as u32, modifier);
            }
        }
    }
}
...
impl ModifierKeyState {
    ...
    pub fn key_event(&mut self, state: ElementState, keycode: u32, modifier: Modifier) {
        match state {
    ...
    pub fn key_press(&mut self, keycode: u32, modifier: Modifier) {
