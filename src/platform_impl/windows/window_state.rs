...
use winapi::Windows::Win32::{
    ...
    UI::{
        DisplayDevices::RECT,
        WindowsAndMessaging::{
            SendMessageW, SetWindowLongW, SetWindowPos, ShowWindow, GWL_EXSTYLE, GWL_STYLE, HWND,
            HWND_NOTOPMOST, HWND_TOPMOST, LPARAM, SWP_ASYNCWINDOWPOS, SWP_FRAMECHANGED,
            SWP_NOACTIVATE, SWP_NOMOVE, SWP_NOSIZE, SWP_NOZORDER, SW_HIDE, SW_MAXIMIZE,
            SW_MINIMIZE, SW_RESTORE, SW_SHOW, WINDOWPLACEMENT, WINDOW_EX_STYLE, WINDOW_STYLE,
            WPARAM, WS_BORDER, WS_CAPTION, WS_CHILD, WS_CLIPCHILDREN, WS_CLIPSIBLINGS,
            WS_EX_ACCEPTFILES, WS_EX_APPWINDOW, WS_EX_NOREDIRECTIONBITMAP, WS_EX_TOPMOST,
...
impl WindowFlags {
    ...
    pub fn to_window_styles(self) -> (WINDOW_STYLE, WINDOW_EX_STYLE) {
        let (mut style, mut style_ex) = (WINDOW_STYLE::default(), WINDOW_EX_STYLE::default());
    ...
    fn apply_diff(mut self, window: HWND, mut new: WindowFlags) {
        ...
        if diff.contains(WindowFlags::ALWAYS_ON_TOP) {
            unsafe {
                SetWindowPos(
                    window,
                    if new.contains(WindowFlags::ALWAYS_ON_TOP) {
                        HWND_TOPMOST
                    } else {
                        HWND_NOTOPMOST
                    },
