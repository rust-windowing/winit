...
use winapi::Windows::Win32::{
    ...
    UI::{
        ...
        WindowsAndMessaging::{
            HWND, RI_MOUSE_LEFT_BUTTON_DOWN, RI_MOUSE_LEFT_BUTTON_UP, RI_MOUSE_MIDDLE_BUTTON_DOWN,
...
pub fn get_raw_input_device_name(handle: HANDLE) -> Option<String> {
    ...
    let mut name = Vec::<u16>::with_capacity(minimum_size as _);
    ...
    Some(unsafe { OsString::from_wide(&name) }.into_string().unwrap())
}
...
pub fn register_raw_input_devices(devices: &[RAWINPUTDEVICE]) -> bool {
    ...
    let success =
        unsafe { RegisterRawInputDevices(devices.as_mut_ptr(), devices.len() as _, device_size) };
...
pub fn register_all_mice_and_keyboards_for_raw_input(window_handle: HWND) -> bool {
    ...
    // HID_USAGE values are missing: https://github.com/microsoft/win32metadata/issues/428
