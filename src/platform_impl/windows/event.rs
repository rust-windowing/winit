...
use winapi::Windows::Win32::{
    ...
    UI::WindowsAndMessaging::{
        MAPVK_VK_TO_CHAR, VK_ADD, VK_APPS, VK_BACK, VK_BROWSER_BACK, VK_BROWSER_FAVORITES,
        VK_BROWSER_FORWARD, VK_BROWSER_HOME, VK_BROWSER_REFRESH, VK_BROWSER_SEARCH,
        VK_BROWSER_STOP, VK_CAPITAL, VK_CONTROL, VK_CONVERT, VK_DECIMAL, VK_DELETE, VK_DIVIDE,
        ...
        VK_F6, VK_F7, VK_F8, VK_F9, VK_HOME, VK_INSERT, VK_KANA, VK_KANJI, VK_LAUNCH_MAIL,
        ...
        VK_MULTIPLY, VK_NEXT, VK_NONCONVERT, VK_NUMLOCK, VK_NUMPAD0, VK_NUMPAD1, VK_NUMPAD2,
    ...
    UI::{TextServices::HKL, WindowsAndMessaging::MAPVK_VSC_TO_VK_EX},
};
...
unsafe fn get_char(keyboard_state: &[u8; 256], v_key: u32, hkl: HKL) -> Option<char> {
    ...
    let len = ToUnicodeEx(
        ...
        PWSTR(unicode_bytes.as_mut_ptr()),
...
fn layout_uses_altgr() -> bool {
    unsafe {
        ...
        let old_hkl = ACTIVE_LAYOUT.swap(hkl.0, Ordering::SeqCst);
...
pub fn handle_extended_keys(vkey: u32, mut scancode: u32, extended: bool) -> Option<(u32, u32)> {
    ...
    let vkey = match vkey {
        VK_SHIFT => unsafe { MapVirtualKeyA(scancode, MAPVK_VSC_TO_VK_EX) },
        VK_CONTROL => {
            if extended {
                VK_RCONTROL
            } else {
...
pub fn process_key_params(wparam: u32, lparam: u32) -> Option<(ScanCode, Option<VirtualKeyCode>)> {
    let scancode = (lparam >> 16) & 0xff;
    let extended = (lparam & 0x01000000) != 0;
    handle_extended_keys(wparam, scancode, extended)
...
fn map_text_keys(win_virtual_key: u32) -> Option<VirtualKeyCode> {
