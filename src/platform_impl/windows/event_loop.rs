...
impl EventLoop {
    ...
    fn wait_for_messages(&mut self, timeout: Option<Duration>) {
        ...
        if timeout.is_some() && self.high_resolution_timer.is_none() {
            self.high_resolution_timer = create_high_resolution_timer();
        }

        let high_resolution_timer: Option<RawHandle> =
            self.high_resolution_timer.as_ref().map(OwnedHandle::as_raw_handle);
        ...
        unsafe {
            ...
            //  2. We failed to get and set high resolution timer and we need something instead of
            //     it.
            let wait_duration_ms = timeout.map(dur2timeout).unwrap_or(INFINITE);

            let mut raw_handles: [RawHandle; 1] = [ptr::null_mut()];
            let (num_handles, handles_ptr) = if use_timer {
                raw_handles[0] = high_resolution_timer.unwrap();
                (1, raw_handles.as_ptr())
            } else {
                (0, ptr::null())
            };
