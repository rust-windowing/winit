...
use winit_core::event::{
    DeviceButtonSource, DeviceEvent, DeviceId, ElementState, PointerSource, RawKeyEvent,
    ...
    fn set_listener(&self) {
        ...
        *self.0.on_mouse_move.borrow_mut() = Some(EventListenerHandle::new(
            ...
            Closure::new(move |event: PointerEvent| {
                ...
                if let Some(button_id) = backend::event::raw_button(&event) {
                    let state = if backend::event::pointer_buttons(&event)
                        .contains(ButtonsState::from_bits_retain(button_id))
                    {
                    ...
                    let source = match backend::event::pointer_source(&event, kind) {
                        PointerSource::Mouse => {
                            DeviceButtonSource::Mouse(backend::event::mouse_button(button_id))
                        },
                        PointerSource::Touch { finger_id, force } => {
                            DeviceButtonSource::Touch { finger_id, force }
                        },
                        PointerSource::TabletTool { kind, data } => {
                            DeviceButtonSource::TabletTool {
                                kind,
                                button: backend::event::tool_button(button_id),
                                data,
                            }
                        },
                        PointerSource::Unknown => DeviceButtonSource::Unknown,
                    };
        ...
        *self.0.on_mouse_press.borrow_mut() = Some(EventListenerHandle::new(
            ...
            Closure::new(move |event: PointerEvent| {
                ...
                let kind = backend::event::pointer_kind(&event, pointer_id);
                let button_id =
                    backend::event::raw_button(&event).expect("no pointer button pressed");
                let source = match backend::event::pointer_source(&event, kind) {
                    PointerSource::Mouse => {
                        DeviceButtonSource::Mouse(backend::event::mouse_button(button_id))
                    },
                    ...
                    PointerSource::TabletTool { kind, data } => DeviceButtonSource::TabletTool {
                        kind,
                        button: backend::event::tool_button(button_id),
                        data,
                    },
