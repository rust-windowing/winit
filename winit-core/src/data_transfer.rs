...
//! - [`OutgoingDragEnded`](crate::event::WindowEvent::OutgoingDragEnded) - the application receives
//!   this when the user has ended the drag operation, either by dropping the data or by canceling
//!   the operation
//!
//! # Detailed flow
//!
...
//!   [`OutgoingDragEnded`](crate::event::WindowEvent::OutgoingDragEnded) with the result of the
//!   drag, and handles it appropriately. For example, if the drag was successful and the operation
//!   is [`DndAction::Move`](crate::event_loop::DndAction::Move), then the application would delete
//!   the source object, since the data has now been transfered somewhere else.
