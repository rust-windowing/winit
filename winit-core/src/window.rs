...
pub trait Window: AsAny + Send + Sync + fmt::Debug {
    ...
    /// method events.
    ///
    /// The return value is `true` if there was an input method actually available to act on.
    ///
...
impl ImeState {
    ...
    ///
    /// On Wayland, this the cursor area should be set from the first request. Otherwise, the input
